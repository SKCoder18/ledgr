...
export default function CategoryChart({ expenses, currency }: { expenses: Expense[]; currency: string }) {
  const data = useMemo(() => {
    ...
    const map = expenses.filter((e) => new Date(e.expense_date) >= monthStart).reduce<Record<string, number>>((acc, e) => { acc[e.category] = (acc[e.category] || 0) + Number(e.amount); return acc; }, {});
